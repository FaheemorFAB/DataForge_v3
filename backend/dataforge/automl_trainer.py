...
FIXES (2026-03):
  • _encode_features: select_dtypes now includes 'object' AND 'category' (FIX 1).
  • cat.codes -1 → np.nan so median imputation works correctly (FIX 2).
  • _build_leaderboard: uses best_loss_per_estimator / best_config_per_estimator
    instead of the non-existent model_history attribute (FIX 3).
  • get_json_result(): new helper that strips model_pkl (bytes) so the Flask
    route can safely call jsonify() without a TypeError 500 (FIX 4).
"""
...
def _build_leaderboard(automl) -> list:
    """
    FIX 3: automl.model_history does not exist in FLAML — it was a fabricated
...
def run_automl(
    ...
    IMPORTANT: The returned dict contains 'model_pkl' (bytes).
    Call get_json_result(result) before jsonify() to strip it safely (FIX 4).
    """
    try:
    ...
    try:
        ...
        return {"error": f"FLAML training failed: {str(e)}", "model_pkl": None}
    ...
    return {
        ...
        "model_pkl":          model_pkl,   # bytes — NOT JSON-serialisable; use get_json_result()
    }


def get_json_result(result: dict) -> dict:
    """
    FIX 4 — Return a JSON-safe copy of the run_automl() result.

    Strips 'model_pkl' (bytes) which is never JSON-serialisable.
    Call this before jsonify():

        result   = run_automl(df, target)
        model_bytes = result.get("model_pkl")
        if model_bytes:
            _save("automl_model", model_bytes)   # persist to session
        return jsonify(get_json_result(result))  # safe ✓
    """
    safe = {k: v for k, v in result.items() if k != "model_pkl"}
    return safe