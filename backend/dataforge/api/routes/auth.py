...
_COOKIE_OPTS = dict(httponly=True, samesite="lax", secure=False, path="/")  # set secure=True behind HTTPS
...
async def auth_google_callback(request: Request):
    if not settings.GOOGLE_AUTH_ENABLED:
        return RedirectResponse(url="http://localhost:3000/?login=1&error=oauth_disabled")
    ...
    if not userinfo:
        return RedirectResponse(url="http://localhost:3000/?login=1&error=oauth")
