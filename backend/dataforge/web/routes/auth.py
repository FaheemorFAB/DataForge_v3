...
def auth_google_callback():
    ...
    try:
        ...
        return redirect("http://localhost:3000/?login=1&error=oauth")
    ...
    try:
        ...
        if user_data is None:
            ...
            if not res:
                ...
                return redirect("http://localhost:3000/?login=1&error=db")
            user = User(**res)
        else:
        ...
        if isinstance(next_url, str) and next_url.startswith("/"):
            return redirect(f"http://localhost:3000{next_url}")
