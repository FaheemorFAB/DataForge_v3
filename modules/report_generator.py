def _format_summary_html(text: str) -> str:
    ...
    if is_numbered:
        ...
        for line in lines:
            ...
            title_html = '<span style="font-weight:700;font-size:13px;color:#E2E8F0">' + title + '.</span>'
            desc_html  = (' <span style="font-size:13px;color:#94A3B8">' + desc + '</span>') if desc else ''
            row = (
    ...
    for p in paras:
        if p.strip():
            result += '<p style="color:#CBD5E1;font-size:15px;line-height:1.75;margin-bottom:10px">' + p.replace("\n", " ").strip() + '</p>'
    return result
...
Report Generator
════════════════
Converts insight dicts + a narrative summary into a self-contained HTML report.
The HTML is stored in the DB/filesystem and served as a preview or download.
...
Usage::
    ...
    html = generate_html_report(
        insights      = [...],
        summary_text  = "Revenue grew 12%...",
        dataset_name  = "sales_q1.csv",
        dataset_type  = "sales",
        profile       = {"rows": 1200, "cols": 8, "missing_pct": 1.2},
    )
"""
...
def _badge(insight_type: str) -> str:
    ...
    return (
        ...
        f'padding:2px 8px;border-radius:12px;letter-spacing:0.5px">{label}</span>'
    )
...
def _chart_canvas(idx: int, insight: dict) -> str:
    ...
    if ctype == "scatter":
        xs     = cd.get("x", [])
        ys     = cd.get("y", [])
        points = ", ".join(f"{{x:{x},y:{y}}}" for x, y in zip(xs, ys))
        data_js = (
        ...
        opts_js = "{scales:{x:{title:{display:true,text:'" + cd.get("x_label","x") + "'}},y:{title:{display:true,text:'" + cd.get("y_label","y") + "'}}}}"
        chart_type = "scatter"
        ...
        lbl_js  = str(labels).replace("'", '"')
        val_js  = str(values)
        data_js = (
            f"{{labels:{lbl_js},"
            f"datasets:[{{label:'{insight.get('metric','')}',data:{val_js},"
...
def generate_html_report(
    ...
    <div style="background:#1A1A2E;border:1px solid #2A2A4A;border-radius:12px;
      ...
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <h3 style="margin:0;font-size:16px;color:#E2E8F0">{ins['title']}</h3>
        {_badge(ins.get('type',''))}
      </div>
...
<head>
  ...
  <title>DataForge Report — {dataset_name}</title>
...
<body>
  ...
  <div style="background:linear-gradient(135deg,#1a1a3e,#0D0D1A);
    ...
    <div style="max-width:900px;margin:0 auto">
      ...
      <h1 style="font-size:22px;color:#E2E8F0;font-weight:600">{dataset_name}</h1>
