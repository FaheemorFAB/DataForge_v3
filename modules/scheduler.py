...
Usage::
    ...
    from modules.scheduler import scheduler, init_scheduler
    init_scheduler(app)

    # To schedule a report:
    from modules.scheduler import add_report_job
...
def _reload_all_jobs(app):
    ...
    try:
        from modules.models import ReportSchedule
...
def _run_scheduled_report(app, schedule_id: int):
    ...
    with app.app_context():
        try:
            from modules.models import db, ReportSchedule, Upload, Report, Alert
            from modules.insight_engine import detect_schema, run_insights, summarise_with_gemini
            ...
            project_dir = pathlib.Path(f"instance/projects/{upload.id}")
...
def add_report_job(app, upload_id: int, cron: str, email: str = "", slack_webhook: str = "") -> int:
    ...
    """
    from modules.models import db, ReportSchedule
